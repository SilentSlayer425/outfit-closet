 ...
 * Weather Widget
 *
 * Collapsible 6-hour weather forecast for the outfit builder.
...
export function WeatherWidget({ savedCity, onCityChange }: WeatherWidgetProps) {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${coords.lat}&longitude=${coords.lon}&hourly=temperature_2m,weather_code&forecast_hours=${FORECAST_HOURS}&temperature_unit=fahrenheit`
    )
      .then((r) => r.json())
      .then((data) => {
        if (cancelled) return;
  ...
  const selectCity = (r: GeoResult) => {
    setCity(`${r.name}, ${r.country}`);
    setCoords({ lat: r.latitude, lon: r.longitude });
    setResults([]);
    setQuery('');
    onCityChange(`${r.name}, ${r.country}`, r.latitude, r.longitude);
  };
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
              {forecast.map((h) => {
                ...
                return (
                  <div key={h.time} className="flex flex-col items-center gap-1 rounded-xl bg-muted/40 p-2">
                    <span className="text-xs text-muted-foreground">{hour}</span>
                    <span className="text-xl">{weatherEmoji(h.weatherCode)}</span>
                    <span className="text-sm font-semibold text-foreground">{Math.round(h.temp)}°F</span>
                  </div>
                );
              })}
            </div>
